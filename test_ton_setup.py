...
# На хосте может быть корень проекта
if os.path.exists('buyer'):
    # Запуск с корня проекта (локально)
    sys.path.insert(0, 'buyer')
    settings_module = 'buyer.settings'
else:
    # Запуск из папки buyer (в контейнере)
    sys.path.insert(0, '.')
    settings_module = 'buyer.settings'
...
def check_ton_setup():
    ...
    try:
        ...
        print(f"    Code cell hash: {code_cell.hash.hex()[:16]}...")
        print(f"    Code cell size: {len(code_cell.serialize())} bytes")
